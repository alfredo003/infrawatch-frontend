  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-neutral-800 dark:text-neutral-200">Relatórios e SLA</h1>
          <p className="text-sm text-neutral-600 dark:text-neutral-400">Análise de desempenho e cumprimento de SLA</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-neutral-600 dark:text-neutral-400">INCIDENTES</p>
        ...
        <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-neutral-600 dark:text-neutral-400">MTTR MÉDIO</p>
                <p className="text-3xl font-bold text-blue-600">45min</p>
                <div className="flex items-center gap-1">
                  <TrendingDown className="w-3 h-3 text-green-600" />
                  <p className="text-xs text-green-600">-7min vs mês anterior</p>
                </div>
              </div>
        ...
        <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-neutral-600 dark:text-neutral-400">DISPONIBILIDADE</p>
          ...
          <div className="space-y-6">
            {slaReports.map((service, index) => (
              <div key={index} className="space-y-2">
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="font-medium text-neutral-800 dark:text-neutral-200">{service.service}</h3>
                    <p className="text-sm text-neutral-600 dark:text-neutral-400">
                      Meta: {service.target}% | Atual: {service.actual}% | Incidentes: {service.incidents}
                    </p>
                  </div>
                  <div className="text-right">
                    <span
                      className={`text-sm font-medium ${
                        service.actual >= service.target ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      {service.actual >= service.target ? "✓ Cumprido" : "✗ Não Cumprido"}
                    </span>
                  </div>
                </div>
                <div className="relative">
                  <Progress value={service.actual} className="h-3" />
                  <div className="absolute top-0 h-3 w-0.5 bg-red-500" style={{ left: `${service.target}%` }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {monthlyReports.map((report, index) => (
                ...
                >
                  <div>
                    <h3 className="font-medium text-neutral-800 dark:text-neutral-200">{report.month}</h3>
                    <p className="text-sm text-neutral-600 dark:text-neutral-400">
                      {report.incidents} incidentes • MTTR: {report.mttr}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold text-neutral-800 dark:text-neutral-200">{report.uptime}%</p>
          ...
          <CardContent>
            <div className="h-64 relative">
              ...
              <svg className="absolute inset-0 w-full h-full">
                <polyline points="0,180 100,160 200,170 300,150 400,140" fill="none" stroke="#16a34a" strokeWidth="3" />
              </svg>
