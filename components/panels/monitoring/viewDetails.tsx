...
export default function ModalViewSystem({
  selectedServer,
  setSelectedServer,
}: ModalProps) {
  return (
      ...
      <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700 shadow-2xl w-full h-full flex flex-col">
        ...
        <CardContent className="p-4 sm:p-6 flex-grow flex flex-col space-y-6 overflow-hidden">
          <div className="flex flex-col md:flex-row gap-4 sm:gap-6 flex-grow">
            ...
            <div className="flex-1 flex flex-col space-y-4 sm:space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  <Badge
                    className={`${getStatusColor(selectedServer.status)} font-medium text-xs sm:text-sm`}
                  >
              ...
              <div className="flex-1 flex flex-col">
                <p className="text-xs sm:text-sm text-neutral-600 dark:text-neutral-400 font-medium mb-2">
                ...
                <div className="flex-1 overflow-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead className="text-neutral-600 dark:text-neutral-400 text-xs sm:text-sm">
                          Tipo
                        </TableHead>
                        <TableHead className="text-neutral-600 dark:text-neutral-400 text-xs sm:text-sm">
                          Métrica
                    ...
                    <TableBody>
                      {connectionTypes
                        .filter(
                          (conn) =>
                            conn.type === selectedServer.connection_type,
                        ) // ajustado
                        .flatMap((conn) =>
                          conn.metrics.map((metric) => (
                            <TableRow key={`${conn.type}-${metric.key}`}>
                              ...
                              <TableCell className="text-neutral-800 dark:text-neutral-200 font-mono text-xs sm:text-sm">
                                {selectedServer?.metric?.[
                                  metric?.key as keyof typeof selectedServer.metric
                                ] ?? 0}{' '}
                                {metric.unit}
                              </TableCell>
                            </TableRow>
            ...
            {/* DIAGRAMA */}
            <div className="flex-1 flex flex-col">
              <p className="text-xs sm:text-sm text-neutral-600 dark:text-neutral-400 mb-2 font-medium">
                DIAGRAMA DE PORTAS
              </p>
