...
interface ListProps {
  ...
  const getImgByTypeConexao = (type: string) => {
    switch (type) {
      case "api":
        return <Globe className="w-5 h-5 text-neutral-600 dark:text-neutral-400" />;
      case "snmp":
        return <HardDrive className="w-5 h-5 text-neutral-600 dark:text-neutral-400" />;
      case "ping":
        return <Router className="w-5 h-5 text-neutral-600 dark:text-neutral-400" />;
      case "webhook":
        return <Link className="w-5 h-5 text-neutral-600 dark:text-neutral-400" />;
      default:
        return <Globe className="w-5 h-5 text-neutral-600 dark:text-neutral-400" />;
    }
  };
  
export default function ListSystems({Systems,
  loading = false,
  onSystemReload,
}: ListProps)
{
