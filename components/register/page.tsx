...
import { User, Mail, Lock, UserCheck, Eye, EyeOff, Users, Shield, Activity, Clock } from "lucide-react"
...
export default function RegisterPage() {
  ...
  useEffect(()=>{
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/users`,{headers: getAuthHeaders()})
    .then((response) => response.json())
    .then((data) => {
        setRecentUsers(data.data || [])
        const totalUsers = data.data?.length || 0;
        const totalAdmins = data.data?.filter((user) => user.role === "admin").length || 0;
        const totalOperators = data.data?.filter((user) => user.role === "operator").length || 0;
        const totalViewers = data.data?.filter((user) => user.role === "viewer").length || 0;
  ...
  const filteredUsers = fullUsers.filter((u: any) => {
    ...
    const matchesQuery = !q || `${u.username} ${u.email}`.toLowerCase().includes(q);
    return matchesRole && matchesQuery;
  });

  const handleExportCSV = () => {
    const rows = [['id','username','email','role']];
    fullUsers.forEach(u => rows.push([u.id ?? '', u.username ?? '', u.email ?? '', u.role ?? '']));
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-wider">REGISTRO DE USUÁRIOS</h1>
          <p className="text-sm text-neutral-400">Gestão e cadastro de novos operadores do sistema</p>
        </div>
        <div className="flex gap-2">
          <Dialog open={openUsers} onOpenChange={setOpenUsers}>
            <DialogTrigger asChild>
              <Button className="bg-blue-500 hover:text-blue-600 text-white">Ver Lista Completa</Button>
            </DialogTrigger>
            ...
            <DialogContent>
              <DialogHeader>
                <div className="flex items-center justify-between w-full">
                  ...
                  <div className="flex items-center gap-2">
                    <Button onClick={handleRefreshFull} className="bg-neutral-700 hover:bg-neutral-600 text-white text-xs">Atualizar</Button>
                    <Button onClick={handleExportCSV} className="bg-neutral-700 hover:bg-neutral-600 text-white text-xs">Exportar CSV</Button>
                    <DialogClose asChild>
                      <Button className="bg-neutral-700 hover:bg-neutral-600 text-white">Fechar</Button>
                    </DialogClose>
                  </div>
                </div>
              </DialogHeader>
              ...
              <div className="mt-4 flex gap-2">
                ...
                />
                <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="bg-neutral-800 border border-neutral-700 rounded px-3 py-2 text-sm text-white font-mono">
                ...
                {isLoadingFull ? (
                  ...
                  filteredUsers.length ? (
                    filteredUsers.map((u: any) => (
                      <div key={u.id || u.email} className="p-2 rounded hover:bg-neutral-800">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <span className={getRoleColor(u.role) + " px-2 py-0.5 rounded text-xs font-mono"}>{(u.role||'').toUpperCase()}</span>
                            <Button size="sm" onClick={() => handleToggleDetails(u.id)} className="bg-neutral-700 hover:bg-neutral-600 text-white text-xs">Detalhes</Button>
                          </div>
                        </div>
                        {selectedUserId === u.id && (
                    ...
                    <div className="text-sm text-neutral-400">Nenhum usuário encontrado.</div>
                  )
                )}
              </div>
            </DialogContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-neutral-900 border-neutral-700">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-neutral-400 tracking-wider">ADMINISTRADORES</p>
                <p className="text-2xl font-bold text-red-500 font-mono">{totalAdmins}</p>
              </div>
        ...
        <Card className="bg-neutral-900 border-neutral-700">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-neutral-400 tracking-wider">OPERADORES</p>
                <p className="text-2xl font-bold text-blue-500 font-mono">{totalOperators}</p>
              </div>
        ...
        <Card className="bg-neutral-900 border-neutral-700">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-neutral-400 tracking-wider">VISUALIZADORES</p>
                <p className="text-2xl font-bold text-white font-mono">{totalViewers}</p>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="bg-neutral-900 border-neutral-700">
          <CardHeader>
            <CardTitle className="text-lg font-bold text-white tracking-wider">NOVO USUÁRIO</CardTitle>
            <p className="text-sm text-neutral-400">Cadastrar novo operador no sistema</p>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="name" className="text-neutral-300 text-sm font-medium uppercase tracking-wide">
                  USUÁRIO
                </Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="email" className="text-neutral-300 text-sm font-medium uppercase tracking-wide">
                  EMAIL
                </Label>
                <div className="relative">
              ...
              <div className="space-y-2">
                <Label htmlFor="password" className="text-neutral-300 text-sm font-medium uppercase tracking-wide">
                  SENHA
                </Label>
                <div className="relative">
              ...
              <div className="space-y-2">
                <Label htmlFor="role" className="text-neutral-300 text-sm font-medium uppercase tracking-wide">
                  FUNÇÃO
                </Label>
                <div className="relative">
                  ...
                  <Select value={formData.role} onValueChange={(value) => handleInputChange("role", value)}>
                    ...
                    <SelectContent className="bg-neutral-800 border-neutral-600">
                      ...
                      <SelectItem value="operator" className="text-white hover:bg-neutral-700">
                        Operator (exibe e reage a alertas)
                      </SelectItem>
                      <SelectItem value="viewer" className="text-white hover:bg-neutral-700">
                  ...
                  {errors.role && (
                    ...
                    {authSuccess && (
                      <InlineSuccess message={authSuccess} />
                    )}
              <Button
                disabled={isLoading}
                type="submit"
        ...
        <Card className="bg-neutral-900 border-neutral-700">
          <CardHeader>
            <CardTitle className="text-lg font-bold text-white tracking-wider">USUÁRIOS RECENTES</CardTitle>
            <p className="text-sm text-neutral-400">Últimos usuários cadastrados no sistema</p>
          </CardHeader>
          <CardContent>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-neutral-400 font-mono">{user.id}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
