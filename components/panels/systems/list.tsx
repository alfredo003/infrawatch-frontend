  ...
  {loading ? (
    ...
    paginatedSystems.map((system: SystemData) => (
      ...
      >
        ...
        <td className={`py-4 px-2 text-center font-semibold ${getSeverityColor(system.criticality_level)}`}>
          {getSeverityLabel(system.criticality_level)}
          </td>
        <td className="py-4 px-2 uppercase">
             <code className={`px-2 py-1 text-xs rounded-md text-white bg-black`}>{system.target}</code>
        </td>
      ...
      {/* View Dialog */}
      <Dialog open={systemViewDialogOpen} onOpenChange={setSystemViewDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Visualizar Sistema</DialogTitle>
            <DialogDescription>Detalhes do sistema selecionado</DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            <p><b>Nome:</b> {viewingSystem?.name}</p>
            <p><b>Tipo:</b> {viewingSystem?.typeName}</p>
            <p><b>Conexão:</b> {viewingSystem?.connection_type}</p>
            <p><b>Nível:</b> {getSeverityLabel(viewingSystem?.criticality_level || "")}</p>
            <p><b>Alvo:</b> {viewingSystem?.target}</p>
            <p><b>Intervalo:</b> {viewingSystem?.check_interval}s</p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setSystemViewDialogOpen(false)}>
              Fechar
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      {/* Edit Dialog */}
      <Dialog open={systemEditDialogOpen} onOpenChange={setSystemEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
          ...
          <form className="space-y-3">
            ...
            <div>
              ...
              <Input type="number" defaultValue={viewingSystem?.check_interval} />
            </div>
          </form>
          <DialogFooter>
            <Button variant="outline" onClick={() => setSystemEditDialogOpen(false)}>
              Cancelar
            </Button>
