...
const filteredSystems = allSystems.filter(
  ...
  const maintenanceCount = allSystems.filter((s) => s.status === "MAINTENANCE").length;
  ...
  return (
    <div className="p-6 space-y-6 bg-neutral-50 dark:bg-neutral-950 min-h-screen">
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600 font-mono">{onlineCount}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-600 font-mono">{maintenanceCount}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600 font-mono">{offlineCount}</p>
              </div>
      ...
      <div className="space-y-4">
        ...
        <Tabs defaultValue="ping" onValueChange={setSelectedConnectionType} className="w-full">
...
<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
  {filteredSystems.length > 0 ? (
    ...
    <div className="col-span-2 text-center py-12 text-neutral-600 dark:text-neutral-400">
      Nenhum sistema cadastrado por <span className="font-semibold">{selectedConnectionType.toUpperCase()}</span>.
    </div>
  )}
</div>
      ...
      {/* Server Detail Modal */}
      {selectedServer && (
       <ModalViewSystem selectedServer={selectedServer} setSelectedServer={setSelectedServer}/>
      )}
    </div>
  );
}
