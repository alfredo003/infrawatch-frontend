...
export async function generatePDFReport({ metrics, services, incidents, analytics }: PDFReportData): Promise<void> {
  const doc = new jsPDF();
  let y = 20;
  doc.setFontSize(18);
  doc.text("Relatório Executivo de Infraestrutura", 105, y, { align: "center" });
  y += 10;
